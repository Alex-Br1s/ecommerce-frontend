...
const DashboardProducts = () => {
  ...
  //! LOGICA DE FILTRADO POR CATEGORIA Y ORDENAMIENTO
  useEffect(() => {
  ...
  const updateFilters = (newFilter: Record<string, any>) => {
    ...
    if (newFilter.sort.length) params.sort = newFilter.sort
  ...
  const handleFilterChange = (key: string, event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if(key === 'categories') {
      ...
      setSelectedCategory(newFilters.categories || [])
    }
    if(key === 'sort') {
    ...
    filterProducts({categories: newFilters.categories, sort: newFilters.sort})
  }
  //!---------------------------------------------------------------------------
...
/*   const handleChangeSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  productSearch.length > 0
    ? searchedProduct // Si hay búsqueda, mostrar productos buscados
    : filteredProducts.length > 0
    ? filteredProducts // Si hay filtros aplicados, mostrar productos filtrados
    : products; // Mostrar todos los productos por defecto
    ...
    <section className="min-h-full overflow-y-hidden">
      <aside className="sm:flex flex-wrap grid sm:grid-cols-2 grid-cols-1 gap-y-3 items-center gap-x-3 w-[95vw] xs:w-[86%] min-h-24 rounded-md mx-2 xs:mx-auto bg-white dark:bg-[#272727] shadow-md mt-7 pt-2 pb-1 px-3 font-semibold text-gray-700">
        ...
        <div className="flex flex-col w-full" /* className="bg-[#eee] dark:bg-[#1f1f1f] min-w-44 flex items-center gap-x-2 px-3 h-10 rounded-md shadow-md dark:shadow-[#666] dark:shadow-md" */>
          ...
          <select
            onChange={(e) => handleFilterChange('categories', e )}
            multiple
            className="text-sm outline-none rounded-md max-h-40 max-w cursor-pointer bg-[#eee] dark:bg-[#1f1f1f] dark:text-[#ccc] scrollbar-custom"
          >
          ...
          {selectedCategory.length == 0 ||
            (filteredProducts.length == 0 && (
              <p className="mt-1 text-base text-red-500 dark:text-red-400 ">
              ...
              <thead>
                ...
                <tbody className="dark:bg-[#1c1c1c] bg-white">
                  {productsToDisplay.map((product) => (
                    <tr
                      key={product.id}
                      className="text-gray-600 border-b dark:border-gray-700 border-gray-300"
                    >
                      <td className="pl-3 pr-1 py-1 border-r dark:border-gray-700 border-gray-300 rounded-lg">
                        <div className="flex items-center text-base">
                          ...
                          <div>
                            <p className="font-semibold dark:text-[#ccc]">
                              {product.name}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="px-4 py-1 text-xl dark:text-[#ccc] dark:border-gray-700 border-gray-300 rounded-lg">
                        <div className="flex gap-x-3">
                          <button
                            ...
                            className="text-green-600 sm:text-inherit sm:hover:text-green-600 transition-all duration-300"
                          >
                            <TbEdit />
                          </button>
                          <button
                            onClick={() => openDeleteDialog(product.id)}
                            className="text-red-500 sm:text-inherit sm:hover:text-red-500 transition-all duration-300"
                          >
                            <MdDelete />
                          </button>
                          <button className="text-blue-500 sm:text-inherit sm:hover:text-blue-500 transition-all duration-300">
                            <GrView
                              onClick={() => openDetailDialog(product.id)}
                            />
                          </button>
                        </div>
                      </td>
